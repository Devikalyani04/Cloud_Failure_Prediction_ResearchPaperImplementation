...
Original file is located at
    https://colab.research.google.com/drive/1B3oufABPHMTXEzFqd16owLHL3uQC7fHy
"""
...
==============================================================================
Paper: "Cloud failure prediction based on traditional machine learning and
        deep learning" - Journal of Cloud Computing (2022)
Authors: Tengku Nazmi Tengku Asmawi, Azlan Ismail, Jun Shen

Implementation: PyTorch + Scikit-learn
Environment: Google Colab (GPU)
Dataset: Google Cluster Traces (GCT) 2011

==============================================================================
HOW TO GET THE DATASET & RUN THIS CODE
==============================================================================

STEP 1 — UPLOAD THIS FILE TO GOOGLE COLAB
  a) Go to https://colab.research.google.com
  b) Click "File" → "Upload notebook" → switch to "Upload" tab
     OR just open a new notebook and use the file manager to upload this .py
  c) In a Colab cell, run:
        !python cloud_failure_prediction.py

STEP 2 — ENABLE GPU IN COLAB
  a) Click "Runtime" → "Change runtime type"
  b) Set "Hardware accelerator" to "T4 GPU"
  c) Click "Save"

STEP 3 — GET THE GOOGLE CLUSTER TRACES (GCT) 2011 DATASET
  The dataset is publicly available from Google. Run these commands in a
  Colab cell BEFORE running this script:

  Option A — Download a usable sample (recommended for Colab free tier):
  ------------------------------------------------------------------
    !pip install google-cloud-storage 2>/dev/null
    # The files are stored publicly at:
    # https://commondatastorage.googleapis.com/clusterdata-2011-2/
    #
    # Download job_events and task_events (partitioned CSV files):
    import subprocess, os
    os.makedirs("clusterdata/job_events", exist_ok=True)
    os.makedirs("clusterdata/task_events", exist_ok=True)
    # Download first few partitions only (full dataset = ~400GB)
    for i in range(5):   # increase range for more data
        url = f"https://commondatastorage.googleapis.com/clusterdata-2011-2/job_events/part-{i:05d}-of-00500.csv.gz"
        subprocess.run(["wget", "-q", "-P", "clusterdata/job_events/", url])
    for i in range(10):  # task events are larger
        url = f"https://commondatastorage.googleapis.com/clusterdata-2011-2/task_events/part-{i:05d}-of-00500.csv.gz"
        subprocess.run(["wget", "-q", "-P", "clusterdata/task_events/", url])

  Option B — Use gsutil (faster):
  ------------------------------------------------------------------
    !pip install gsutil
    !gsutil -m cp "gs://clusterdata-2011-2/job_events/part-00000*" ./clusterdata/job_events/
    !gsutil -m cp "gs://clusterdata-2011-2/task_events/part-00000*" ./clusterdata/task_events/

STEP 4 — INSTALL DEPENDENCIES (run in a Colab cell):
  !pip install torch scikit-learn imbalanced-learn pandas numpy matplotlib seaborn tqdm

STEP 5 — RUN THE SCRIPT:
  !python cloud_failure_prediction.py

  OR paste the entire script into a Colab cell and run it directly.

NOTES:
  - The script auto-detects GPU/CPU.
  - If data directories are missing it will generate SYNTHETIC data so you
    can verify the pipeline end-to-end immediately.
  - Set USE_SYNTHETIC_DATA = False once you have the real GCT files.
  - Adjust DATA_SAMPLE_SIZE to control how many rows are loaded (memory).
...
# CONFIGURATION
# ─────────────────────────────────────────────────────────────────────────────
JOB_EVENTS_DIR   = "/content/drive/MyDrive/clusterdata/job_events"   # ← real path
TASK_EVENTS_DIR  = "/content/drive/MyDrive/clusterdata/task_events"  # ← real path
USE_SYNTHETIC_DATA = False   # ← set to False
...
def main():
    ...
    # ── 5. Final findings summary (paper Section 5 / Conclusion) ─────────────
    print(f"\n{'='*70}")
    print("  KEY FINDINGS (consistent with paper)")
    print(f"{'='*70}")
    print("""
  Job-Level Failure Prediction:
    • XGBoost should produce the best accuracy (~93%) with highest
      precision and F-score among all models.
    • DT and RF follow closely.
    • LSTM models perform slightly below TML (excluding LR).
    • Most important features: disk_space_request, cpu_request (XGBoost),
      scheduling_class and cpu_request (most TML models).

  Task-Level Failure Prediction:
    • Decision Tree and Random Forest produce the best models (~89-90%).
    • Priority is the most important feature across all models.
    • memory_request is key for most TML resource-request features.
    • LSTM models score between 86–87%.

  Scalability:
    • Logistic Regression is the most scalable TML model
      (prediction in <1s for 1M rows; <2s for 10M rows).
    • DL models are ~1000x slower per prediction than TML.
""")
